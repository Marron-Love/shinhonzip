...
const routeUrl = env.ROUTE_STATIONS_API_URL || "https://apis.data.go.kr/6410000/busrouteservice/v2/getBusRouteStationListv2";
...
const routeIds = (env.BUS_ROUTE_IDS || "").split(",").map((v) => v.trim()).filter(Boolean);
...
const dateKey = (value) => {
  ...
  return textDate || (Number.isNaN(parsed.getTime()) ? "" : ymd(parsed));
};
...
function normalizeLocation(row) {
  return {
    ...
    retrieved_at: row.retrieved_at ?? row.retrievedAt ?? row.timestamp ?? iso(now),
    remain_seat_num: Number(row.remain_seat_num ?? row.remainSeatNum ?? row.remaining_seats ?? row.remainingSeats ?? 0),
...
async function fetchRouteStations(routeId) {
  ...
  const request = new URL(routeUrl);
  for (const [key, value] of Object.entries(params)) request.searchParams.set(key, value);
...
async function update() {
  ...
  if (!locationUrl || !routeUrl) throw new Error("LOCATION_API_URL and ROUTE_STATIONS_API_URL are required.");

  const end = now;
  const start = new Date(end.getTime() - hours * 60 * 60 * 1000);
  const locationRows = arrayFrom(await fetchJson(locationUrl, { [startParam]: iso(start), [endParam]: iso(end) }))
    .map(normalizeLocation)
    .filter((row) => row.route_id && row.station_seq && row.retrieved_at);

  for (const [date, rows] of splitByDate(locationRows)) {
    const file = path.join(locationDir, `${date}.json`);
    await writeJson(file, dedupe([...await readJson(file, []), ...rows]).sort((a, b) => parseDate(a.retrieved_at) - parseDate(b.retrieved_at)));
  }

  const routes = [...new Set([...routeIds, ...locationRows.map((row) => row.route_id)])].sort();
  await updateRoutes(routes);

  const oldIndex = await readJson(path.join(dataDir, "index.json"), { dates: [] });
  const dates = [...new Set([...(oldIndex.dates || []), ...splitByDate(locationRows).keys()])].sort();
...
async function updateRoutes(routes) {
  ...
  for (const routeId of routes) {
    ...
    if (stations.length) await writeJson(path.join(routeDir, `${routeId}.json`), stations);
  }
}
...
function selfCheck() {
  ...
  assert.deepEqual([...splitByDate(rows).keys()], ["2026-07-12", "2026-07-13"]);
  assert.deepEqual(xmlItems("<busRouteStationList><stationSeq>7</stationSeq><stationId>1</stationId><stationName>테스트</stationName></busRouteStationList>"), [
    { stationSeq: "7", stationId: "1", stationName: "테스트" }
  ]);
}
